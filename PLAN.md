...
7일(1008 tick)마다 퀘스트 갱신. 이 단계에서 플레이어는 아직 "퀘스트를 읽는" 수준.
...
```
deadline = 다음 갱신 tick (issued_tick + 1008).
...
| `quests/generator.py` | `QuestGenerator.tick()` — 1008 tick 주기 |
...
- QuestGenerator (LLM 포함): < 30s / 1008 tick 주기
