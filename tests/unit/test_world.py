...
def test_build_indices(mini_world):
    assert "farmer_1" in mini_world.agents_by_node["farm"]
...
def test_remove_agent(mini_world):
    ...
    assert "farmer_1" not in mini_world.agents_by_node.get("farm", [])
    assert "farmer_1" not in mini_world.agents_by_role.get(Role.FARMER, [])


def test_move_agent_updates_indices(mini_world):
    world_ops.move_agent(mini_world, "farmer_1", "city")
    assert mini_world.agents["farmer_1"].current_node == "city"
    assert "farmer_1" in mini_world.agents_by_node["city"]
    assert "farmer_1" not in mini_world.agents_by_node.get("farm", [])
...
def test_snapshot_read_ops(mini_world):
    ...
    assert snap.tick == 0
    assert snap.get_node("city").name == "City"
    assert snap.get_agent("farmer_1").role == Role.FARMER
    assert len(snap.agents_at("farm")) == 1
